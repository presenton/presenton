...
def test_build_system_prompt_includes_trimmed_memory_blocks():
    ...
    assert (
        "If grouped text or elements still cannot be moved cleanly inside the group"
        in system_prompt
    )
    assert "\nPrior deck decision\n" in system_prompt
    assert "\nEarlier user request\n" in system_prompt
...
def test_system_prompt_preserves_existing_content_for_additive_edits():
    ...
    assert (
        "Use primitive elements only when the task cannot be achieved from existing blocks"
        in system_prompt
    )
    ...
    assert (
        "Prefer this over fetching a whole layout or schema just to find one block" in system_prompt
    )
    assert (
        "For new table or chart requests, getAvailableBlocks is mandatory before inserting"
        in system_prompt
    )
    assert (
        "Do not use addElement to create a new table or chart while a reusable block exists"
        in system_prompt
    )
    ...
    assert (
        "Use deleteElement, deleteComponent, or deleteSlide only when deletion is explicitly requested"
        in system_prompt
    )
