...
def test_system_prompt_requires_content_only_outlines_for_visual_instructions():
    ...
    assert "Never include or paraphrase commands, configuration, or meta-commentary" in prompt
...
def test_generate_ppt_outline_injects_external_search_context_without_hosted_tool():
    ...
    with (
        ...
        patch.object(outline_module, "get_client", return_value=object()),
        patch.object(outline_module, "get_llm_config", return_value={}),
        patch.object(outline_module, "should_use_native_web_search", return_value=False),
        patch.object(outline_module, "should_expose_external_web_search_tool", return_value=True),
        patch.object(
            outline_module,
...
def test_generate_ppt_outline_emits_provider_aware_external_search_statuses():
    ...
    with (
        ...
        patch.object(outline_module, "generate_web_search_query", return_value="current Nepal PM"),
        patch.object(outline_module, "get_web_search_context", return_value="Current facts"),
        patch.object(outline_module, "get_generate_kwargs", side_effect=lambda **kwargs: kwargs),
        patch.object(
            outline_module, "stream_generate_events", side_effect=fake_stream_generate_events
        ),
    ):
...
def test_generate_ppt_outline_emits_model_native_search_status():
    ...
    with (
        ...
        patch.object(outline_module, "get_client", return_value=object()),
        patch.object(outline_module, "get_llm_config", return_value={}),
        patch.object(outline_module, "should_use_native_web_search", return_value=True),
        patch.object(outline_module, "should_expose_external_web_search_tool", return_value=False),
        patch.object(outline_module, "get_generate_kwargs", side_effect=lambda **kwargs: kwargs),
        patch.object(
            outline_module, "stream_generate_events", side_effect=fake_stream_generate_events
        ),
    ...
    assert statuses == ["Searching with model-native web search and drafting outlines"]
